...
app.post('/api/bookings', async (req, res) => {
  ...
  const guests = req.body.guests.trim()
  ...
  if (!transporter) {
    console.log('[dev] Reservation finalized (SMTP not configured):', {
      to: email,
      restaurantName,
      date,
      time,
    ...
    console.log(
      '[dev] No email was sent. Copy .env.example to .env and add SMTP_USER / SMTP_PASS to send real emails.',
    )
    return res.json({
      success: true,
      dev: true,
      message:
        'Booking saved locally. Configure SMTP in .env to send confirmation emails.',
    })
  }
